...
const debug = require('debug')('site:tvcesoir.fr')
      ...
      if (queues.length) {
        await doFetch(queues, (url, res) => {
          ...
          programs.push({
            ...
            season,
            episode,
            actor: casts.filter(c => c.role === 'Acteur').map(c => c.name).slice(0, 3),
            director: casts.filter(c => c.role === 'Réalisateur').map(c => c.name),
            presenter: casts.filter(c => c.role === 'Présentateur').map(c => c.name),
            start,
            stop
          })
      })
