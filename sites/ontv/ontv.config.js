  ...
  parser: function ({ content, date, channel }) {
    ...
    items.forEach(item => {
      ...
      programs.push({
        ...
        starrating: parseStarRating($item),
        productionDate: parseProductionDate($item),
        showIcon: parseShowIcon($item),
        start,
        stop
      });
    });
...
function parseSubtitle($item) {
  return $item('h4 > strong').text().trim();
}

function parseDescription($item) {
  return $item('.tvbody > p').text().trim();
}

function parseCategory($item) {
  return $item('.schedule-attributes-genres > span').text().trim();
}

function parseActors($item) {
  return $item('.single-cast-head').text().trim();
}

function parseEpisode($item) {
  return $item('h5 > strong').text().trim();
}

function parseStarRating($item) {
  const rating = $item('.rating-avg').text().trim();
  return rating ? parseFloat(rating) : null;
}

function parseProductionDate($item) {
  return $item('.schedule-attributes > span').text().trim();
}

function parseShowIcon($item) {
  return $item('.w-100').attr('src').trim();
}
