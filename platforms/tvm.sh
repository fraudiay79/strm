...
fetch_channel_data() {
    ...
    # Extract JWT token
    local jwt_token=$(echo "$html_content" | grep -oE 'data-jwt="[^"]+"' | sed -n 's/.*data-jwt="\([^"]*\)".*/\1/p')
    
    # Extract distribution host
    local dist_host=$(echo "$html_content" | grep -oE 'data-dist-host="[^"]+"' | sed -n 's/.*data-dist-host="\([^"]*\)".*/\1/p')
    
    # Extract alternative distribution host (fallback)
    local dist_host_alt=$(echo "$html_content" | grep -oE 'data-dist-host-alt1="[^"]+"' | sed -n 's/.*data-dist-host-alt1="\([^"]*\)".*/\1/p')
    
    # Extract source relative path
    local src_rel=$(echo "$html_content" | grep -oE 'data-src-rel="[^"]+"' | sed -n 's/.*data-src-rel="\([^"]*\)".*/\1/p')
...
# Try to fetch data for each channel
...
if validate_data "$jwt_tvm" "$dist_host_tvm"; then
    ...
    final_src_rel_tvm="${src_rel_tvm:-/master.m3u8}"
...
if validate_data "$jwt_tvmnews" "$dist_host_tvmnews"; then
    ...
    final_src_rel_tvmnews="${src_rel_tvmnews:-/master.m3u8}"
...
if validate_data "$jwt_tvmsport" "$dist_host_tvmsport"; then
    ...
    final_src_rel_tvmsport="${src_rel_tvmsport:-/master.m3u8}"
